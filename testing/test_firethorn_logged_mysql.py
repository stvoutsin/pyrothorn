		    ...
		    queryid = None
                    ...
                    if (query_duplicates_found<=0):
	            	    ...
	            	    with Timeout(config.sql_timeout):
                                mysqlEng.execute_query_empty(mysql_flush_query, config.mysql_test_database, config.sql_rowlimit, config.sql_timeout)
                                mysqlEng.execute_query_empty("FLUSH TABLES", config.mysql_test_database, config.sql_rowlimit, config.sql_timeout)
                                mysqlEng.execute_query_empty("FLUSH QUERY CACHE", config.mysql_test_database, config.sql_rowlimit, config.sql_timeout)
                        ...
                        try :
                            ...
                            query = query.replace("`","")
                            with Timeout(config.sql_timeout):
                                sqlserverEng.execute_query_empty(sqlserver_flush_query, config.test_database, config.sql_rowlimit, config.sql_timeout)
                                sqlserverEng.execute_query_empty("DBCC FREEPROCCACHE", config.test_database, config.sql_rowlimit, config.sql_timeout)
