	    ...
	    logged_queries=[]
		    ...
		    queryid = None
			    ...
			    with Timeout(config.firethorn_timeout):
				    ...
				    voqry = voQuery.VOQuery(endpointURL= firethorn_tap_service, query=query,  maxrec=config.sql_rowlimit)
                                    voqry.run()
                                else :
                                    voqry = voQuery.VOQuery(endpointURL= firethorn_tap_service, query=query)
                                    voqry.run()
