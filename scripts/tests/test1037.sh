...
chmod a+r "${testbase:?}/test1037-nohup.sh" 
chcon -t svirt_sandbox_file_t "${testbase:?}/test1037-nohup.sh" 
...
echo "*** Run pyrothorn  [test1037-nohup.sh] ***"
...
docker run -i -t \
    ...
    --volume "${testbase:?}/test1037-nohup.sh:/scripts/test1037-nohup.sh" \
    ...
    --link "${username:?}:${userlink:?}" \
       firethorn/pyrothorn:${version:?} bash -c  '/scripts/test1037-nohup.sh'
